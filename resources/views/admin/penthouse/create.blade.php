...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0">Добавить пентрхаус</h1>
      </div><!-- /.col -->
      <div class="col-sm-6">
...
<section class="content">
  <div class="container-fluid">
    ...
    <div class="row">
      <form action="{{ route('admin.penthouse.store')}}" method="post" enctype="multipart/form-data">
        @csrf 
        ...
        <div class="form-group">
            <input type="number" min="0" max="9999999999" name="price" class="form-control" placeholder="Цена" step="0.01" oninput="validity.valid || (value = '');">
          </div>
        ...
        <div class="form-group">
          <select name="user_id" class="form-control category type  select2-hidden-accessible" style="width: 100%;" data-select2-id="1" tabindex="-1" aria-hidden="true">
            <option selected="selected" disabled data-select2-id="">Выберите владельца</option>
            @foreach($users as $user)
            <option value="{{$user->id}}">{{$user->name . " " . $user->surname . " " . $user->patronymic . " + 4 цифры паспорта"}}</option>                
            @endforeach
          </select>
        </div>
